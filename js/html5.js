...
window.onload = function () {
    ...
    function findAndReplaceText(node, searchText, replacementText) {
        console.log("running")
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            for (let i = 0; i < node.childNodes.length; i++) {
                findAndReplaceText(node.childNodes[i], searchText, replacementText);
            }
        }
    }

    // Call the function to replace 'bee-box' with your desired text
    findAndReplaceText(document.body, 'bee-box', 'beebop');
